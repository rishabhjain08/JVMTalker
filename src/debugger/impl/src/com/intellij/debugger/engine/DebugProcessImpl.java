...
public abstract class DebugProcessImpl extends UserDataHolderBase implements DebugProcess {
  ...
  private Map<String, List<VirtualFile>> registeredFiles;
  ...
  protected DebugProcessImpl(Project project) {
    ...
    registeredFiles = new java.util.HashMap<>();
  }

  private boolean containsVirtualFile(List<VirtualFile> files, VirtualFile myFile) {
  ...
  public List<VirtualFile> getAllRegisteredFiles() {
    List<VirtualFile> allRegisteredFiles = new LinkedList<>();
    for (Map.Entry<String, List<VirtualFile>> e : registeredFiles.entrySet()) {
      allRegisteredFiles.addAll(e.getValue());
    }
    return allRegisteredFiles;
  }

  public List<VirtualFile> getRegisteredFilesByName(String name) {
    List<VirtualFile> files = registeredFiles.get(name);
    if (files == null) {
  ...
  public void registerFile(VirtualFile file) {
    List<VirtualFile> files = registeredFiles.get(file.getName());
    if (files == null) {
      files = new LinkedList<>();
      registeredFiles.put(file.getName(), files);
    }
    if (!containsVirtualFile(files, file)) {
      files.add(file);
    }
  }

  public void unregisterFile(VirtualFile myFile) {
    List<VirtualFile> files = registeredFiles.get(myFile.getName());
    if (files != null) {
      Iterator<VirtualFile> itr = files.iterator();
      while (itr.hasNext()) {
        if (myFile.getUrl().equals(itr.next().getUrl())) {
          itr.remove();
          break;
        }
      }
...
//          getSession().getXDebugSession().sessionResumed();
  ...
  private void createVirtualMachine(final DebugEnvironment environment) {
    ...
    boolean scheded = getManagerThread().schedule(new DebuggerCommandImpl() {
      @Override
  ...
  private void afterProcessStarted(final Runnable run) {
    ...
    LOG.debug("impl...exec result = " + myExecutionResult);
